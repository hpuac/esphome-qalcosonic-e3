...
int main(int argc, char **argv) {
  ...
  for (size_t n = 0; n < frame.size(); ++n)
    assert(parse_frame(frame.data(), n, data) == FrameResult::INCOMPLETE);
  auto bad = frame;
  bad[50] ^= 1;
  ...
  bad[1] = bad[2] = 15;
  bad.push_back(0); bad.push_back(0x16); checksum(bad);
  assert(parse_frame(bad.data(), bad.size(), data) == FrameResult::OK);
  ...
  bad[1] = bad[2] = static_cast<uint8_t>(bad.size() - 6); checksum(bad);
